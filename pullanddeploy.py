...
def git_reset():
	...
	f3 = call(GIT_RESET)
def rsyn_cmd():
	RSYNCCMD = '/usr/bin/rsync -vrlptD --delete /opt/www/html/ drt-mkt-webapp-prod-1:/opt/www/html/'
...
if __name__ == '__main__':
	...
	see_remote_commit_ids()
 	...
 	git_pull()
	git_reset()
	logging.info('COMMIT ID:%s pulled to the staging server drt-mkt-webapp-stage-1' %a)
	yes_or_no = raw_input('Do you want to push the COMMIT ID:%s to the Production server ? Type y/n:' %a)
	if yes_or_no == 'y':
		logging.info('Pushing the COMMIT ID: %s to the Production server drt-mkt-webapp-prod-1' %a)
		print 'Pushing the COMMIT ID: %s to the Production server drt-mkt-webapp-prod-1' %a
		#rsyn_cmd()
	elif yes_or_no == 'n':
